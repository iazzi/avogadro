...
namespace Avogadro {
  ...
  void GLWidget::renderAxesOverlay()
  {
    ...
    gluProject(aX.x(), aX.y(), aX.z(), &model[0][0], &proj[0][0], &view[0], &win_x, &win_y, &win_z);
    win_y = height - win_y; // y is inverted
    QGLWidget::renderText(int(win_x)+3, int(win_y)-3, QString("x"));

    gluProject(aY.x(), aY.y(), aY.z(), &model[0][0], &proj[0][0], &view[0], &win_x, &win_y, &win_z);
    win_y = height - win_y; // y is inverted
    QGLWidget::renderText(int(win_x)+3, int(win_y)-3, QString("y"));

    gluProject(aZ.x(), aZ.y(), aZ.z(), &model[0][0], &proj[0][0], &view[0], &win_x, &win_y, &win_z);
    win_y = height - win_y; // y is inverted
    QGLWidget::renderText(int(win_x)+3, int(win_y)-3, QString("z"));
